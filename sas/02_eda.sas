...
/* --------------- Production header ---------------- */
/* Locate this file's folder, then include env & import from there */
%let _pgm = %sysfunc(dequote(&_SASPROGRAMFILE));
%let _dir = %substr(&_pgm,1,%eval(%length(&_pgm)-%length(%scan(&_pgm,-1,'/'))-1));

/* 1) Environment (assigns RAW/WRK/RES, LOANS fileref, plot gpath) */
%include "&_dir/00_env.sas";
   
/* 2) Ensure RAW.LOANS exists (idempotent) */
%if %sysfunc(exist(raw.loans))=0 %then %do;  
  %include "&_dir/01_import.sas";
%end;
...
data loans;
  ...
  /* --- Numeric copies (assumes import created numeric vars) --- */
